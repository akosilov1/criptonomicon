    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input v-model="tiker" @keyup.enter="add()" @keydown="addError = false" type="text" name="wallet"
                id="wallet"
                class="block w-full pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
                placeholder="Например DOGE" />
            </div>
            <div v-if="filteredCoins().length" class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
              <span v-for="coin in filteredCoins()" :key="coin.id" @click="add(coin.name)"
      ...
      <template v-if="tikers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div v-for="tik in filteredTickers()" :key="tik.name" @click="selectTik(tik)"
      ...
      <section class="relative" v-if="sel">
        ...
        <button @click.stop="sel = null" type="button" class="absolute top-0 right-0">
...
export default {
  ...
  methods: {
    getPrice(tickerName, cb) {
      fetch(API_URL.replace("#FSYM#", tickerName))
        .then(resp => resp.json())
        .then(rez => cb(rez));
    },
    subscribeToUpdate(tickerName) {
      this.getPrice(tickerName, (rez) => {
        console.log(rez);
        if (rez.USD) {
          return setInterval(async () => {
            this.getPrice(tickerName, rez => {
              ...
              const currentTicker = this.tikers.find((t) => t.name === tickerName);
              if (rez.Response === "Error") {
                this.tikers = this.tikers.filter((t) => t != currentTicker);
                return;
              }
              currentTicker.price =
    ...
    normalizeGraph() {
      if(this.graph.length > 10) this.graph.shift();
    ...
    filteredTickers() {
      return this.filter ?
        this.tikers.filter(
          (tik) => tik.name.indexOf(this.filter.toUpperCase()) !== -1
        ) :
        this.tikers.slice((this.page - 1) * 6, this.page * 6);
    },
  },
  watch: {
    filter() {
  ...
  created() {
    ...
    if (tickers) {
      ...
      this.tikers.forEach((item) => {
        item.timer = this.subscribeToUpdate(item.name);
      });
    }
    fetch(
      ...
      .then((res) => res.json())
      .then((rez) => {
        for (let coin in rez.Data)
          this.coins.push({
            id: rez.Data[coin].Id * 1,
            name: rez.Data[coin].Symbol,
            FullName: rez.Data[coin].FullName,
          });
      });
  },
};
